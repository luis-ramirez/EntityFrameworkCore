...
namespace Microsoft.EntityFrameworkCore.Storage
{
    public class ValueComparerTest
    {
        ...
        {
            var comparer = (ValueComparer)Activator.CreateInstance(typeof(ValueComparer<>).MakeGenericType(type));
            ...
            Assert.True(comparer.KeyEquals(value1, value1));
            Assert.True(comparer.KeyEquals(value2, value2));
            Assert.False(comparer.KeyEquals(value1, value2));
            Assert.False(comparer.KeyEquals(value2, value1));
            Assert.False(comparer.KeyEquals(value1, null));
            Assert.False(comparer.KeyEquals(null, value2));
            Assert.True(comparer.KeyEquals(null, null));

            Assert.Equal(0, comparer.HashCode(null));
            Assert.Equal(hashCode ?? value1.GetHashCode(), comparer.HashCode(value1));
            Assert.Equal(0, comparer.KeyHashCode(null));
            Assert.Equal(hashCode ?? value1.GetHashCode(), comparer.KeyHashCode(value1));
...
#pragma warning restore 660,661
        ...
        public void GenericCompareTest<T>(T value1, T value2, int? hashCode = null)
        {
            ...
            var keyEquals = comparer.KeyEqualsExpression.Compile();
            var getHashCode = comparer.HashCodeExpression.Compile();
            var getKeyHashCode = comparer.KeyHashCodeExpression.Compile();
        ...
        public void Can_create_new_comparer_composing_existing_comparers()
        {
            ...
            var comparer = new ValueComparer<DeepBinary>(
                (Expression<Func<DeepBinary, DeepBinary, bool>>)CreateAndExpression(bytesComparer.EqualsExpression),
                (Expression<Func<DeepBinary, int>>)CreateHashCodeExpression(bytesComparer.HashCodeExpression),
                (Expression<Func<DeepBinary, DeepBinary, bool>>)CreateAndExpression(bytesComparer.KeyEqualsExpression),
                (Expression<Func<DeepBinary, int>>)CreateHashCodeExpression(bytesComparer.KeyHashCodeExpression));
        ...
        private static LambdaExpression CreateAndExpression(
            LambdaExpression equalsExpression)
        {
            ...
            var firstEquals = ValueComparer.ReplaceEqualsParameters(
                equalsExpression,
                Expression.Call(param1, _getValue0Method),
                Expression.Call(param2, _getValue0Method));

            var secondEquals = ValueComparer.ReplaceEqualsParameters(
                equalsExpression,
        ...
        private static LambdaExpression CreateHashCodeExpression(
            LambdaExpression hashCodeExpression)
        {
            var param = Expression.Parameter(typeof(DeepBinary), "v");

            var firstHashCode = ValueComparer.ReplaceHashCodeParameter(
                hashCodeExpression,
                Expression.Call(param, _getValue0Method));

            var secondHashCode = ValueComparer.ReplaceHashCodeParameter(
                hashCodeExpression,
