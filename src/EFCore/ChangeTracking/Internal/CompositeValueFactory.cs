...
namespace Microsoft.EntityFrameworkCore.ChangeTracking.Internal
{
    ...
    public class CompositeValueFactory : IDependentKeyValueFactory<object[]>
    {
        ...
        {
            var comparers = properties.Select(p => p.GetValueComparer() ?? p.FindMapping()?.Comparer).ToList();
        ...
        private sealed class CompositeCustomComparer : IEqualityComparer<object[]>
        {
            ...
            {
                _equals = comparers.Select(c => c.KeyEquals).ToArray();
                _hashCodes = comparers.Select(c => c.KeyHashCode).ToArray();
            }
